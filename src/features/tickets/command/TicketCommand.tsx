...
export function TicketCommand({ id, onClose }: { id: string; onClose: () => void }) {
  const { tickets, ticketComments, ticketMessages } = useOrbit();
  ...
  return (
    <div onClick={onClose} style={{ position: "fixed", inset: 0, zIndex: 1000, background: "var(--scrim)", backdropFilter: "blur(8px)", WebkitBackdropFilter: "blur(8px)", display: "flex", flexDirection: "column", padding: "28px 32px" }}>
      <div onClick={(e) => e.stopPropagation()} style={{ flex: 1, minHeight: 0, display: "flex", flexDirection: "column", borderRadius: 22, overflow: "hidden", background: "var(--panel-solid)", border: "1px solid var(--hair-3)", boxShadow: "0 40px 120px rgba(0,0,0,0.5)" }}>
        {/* header */}
        <div style={{ padding: "18px 24px", borderBottom: "1px solid var(--hair-2)", display: "flex", alignItems: "flex-start", gap: 16 }}>
          ...
          <SlaChip f={f} />
          <button onClick={onClose} style={{ width: 34, height: 34, borderRadius: 9, border: "1px solid var(--hair-3)", background: "var(--fill-2)", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
